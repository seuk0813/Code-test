  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selected) {
        e.preventDefault();
        setScore((prev) => removeNoteFromScore(prev, selected));
        setSelected(null);
      }
    };
  ...
  return (
    <div className="app">
      ...
      <div className="status-line">
        {isPlaying ? `재생 중… ${playingMeasure !== null ? playingMeasure + 1 : 1}번 마디` : '오선보를 클릭해 음표를 입력하세요.'}
      </div>
      <StaffEditor score={score} selected={selected} onSelectNote={handleSelectNote} onAddNote={handleAddNote} />
    </div>
  );
}
